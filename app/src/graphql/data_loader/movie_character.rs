...
pub type MovieIdsByCharacterIdDataLoader =
    CachedDataLoader<i32, Vec<i32>, MovieIdsDataLoaderBatchByCharacterId>;
pub type CharacterIdsByMovieIdDataLoader =
    CachedDataLoader<i32, Vec<i32>, CharacterIdsDataLoaderBatchByMovieId>;
...
impl BatchFn<i32, Vec<i32>> for MovieIdsDataLoaderBatchByCharacterId {
    ...
    fn load(&self, keys: &[i32]) -> BatchFuture<Vec<i32>, Self::Error> {
        ...
        let movie_characters_data: Vec<MovieCharacter> = match movie_characters
            .filter(character_id.eq_any(keys))
        ...
        future::ready(
            keys.into_iter()
                .map(|v| match movie_ids_by_character_id.get(v) {
